...
//                判断纵向范围
    ...
    static tbDrag(moveBox,dragAreaBox,scalePic = null,scalePicBox = null){
        ...
        dragAreaBox.onmousemove = function (e) {
            let xx,yy;
            if (e.clientX<dragAreaBox.getBoundingClientRect().left+moveBox.clientWidth/2){
                xx = 0;
            }else if(e.clientX>dragAreaBox.getBoundingClientRect().right-moveBox.clientWidth/2){
                xx = dragAreaBox.clientWidth-moveBox.clientWidth;
            }else {
                xx = e.clientX-dragAreaBox.getBoundingClientRect().left-moveBox.clientWidth/2;
            }

            if (e.clientY<dragAreaBox.getBoundingClientRect().top+moveBox.clientHeight/2){
                yy = 0;
            }else if(e.clientY>dragAreaBox.getBoundingClientRect().bottom-moveBox.clientHeight/2){
                yy = dragAreaBox.clientHeight-moveBox.clientHeight;
            }else {
                yy = e.clientY-dragAreaBox.getBoundingClientRect().top-moveBox.clientHeight/2;
            }
            moveBox.style.top = yy+'px';
            moveBox.style.left = xx+'px';

            let scaX = (scalePic.clientWidth-scalePicBox.clientWidth)/(dragAreaBox.clientWidth-moveBox.clientWidth);
            let scaY = (scalePic.clientHeight-scalePicBox.clientHeight)/(dragAreaBox.clientHeight-moveBox.clientHeight);
            if (scalePic!=null){
